...
chan1 = [ Channel(0,-10,10), Channel(1,-10,10), Channel(2,-10,10), Channel(3,-10,10), Channel(4,-10,10), Channel(5,-10,10)]
...
chan2 = [ Channel(0,-600,600), Channel(1,-600,600), Channel(2,-600,600), Channel(3,-600,600), Channel(4,-600,600), Channel(5,-600,600), Channel(6,-600,600)]

#третьего
client3 = ModbusClient(host = '192.168.0.45', port = 502, auto_open = True, auto_close = True, timeout = 0.15)
chan3 = [ Channel(0,-10,10), Channel(1,-10,10), Channel(2,-10,10), Channel(3,-10,10), Channel(4,-10,10), Channel(5,-10,10)]


#создаем массив кривых которые будут отображатся на графике
curves = [Curve(0,20,[255,0,0],"beam current"), Curve(0,8e-5,[255,255,0],"vacuum high"), Curve(0,150,[200,200,0],"T yarmo"), Curve(0,300,[200,100,0],"T plastik"),
          Curve(0,20,[250,100,100],"current 2"), Curve(0,125,[100,100,250],"gas flow"), Curve(0,8e-5,[0,255,255],"vacuum tube"), Curve(0,1e-1,[0,150,130],"vacuum low")]#, Curve(0,1e-2,[100,100,100])
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        for i in range(100):
            ...
            if not os.path.isfile("logs\\"+name): #если такого файла нет, то выходим из цикла и сохраняем имя, если нет то продолжаем (цифра увеличится на единицу)
                self.fname = name
                break
...
#!            b = client1.read_input_registers(chan.addr,1)#функция считывания байтового значения из ацп по протоколу модбас
            ...
            if b is None: #если вернула None то есть какая-то ошибка - скорее всего нет подключения - тогда задаем значение 666
                ...
                if len(b)>0: #функция вернула массив данных, мы запрашивали один канал так что массив размера один если он меньлше чем один задаем значение 666 - ошибка
                    volt.append(toV(b[0],chan.min,chan.max)) #если нет ошибки, то задаем значение функции toV от первого элемента массива (по идее единственного). toV как раз преобразует байтовое значение в вольты, минимальное и максимальное значение берем из канала
                else:
                    print("chan error2")
                    volt.append(666)
...
#!            b = client2.read_input_registers(chan.addr,1)
        ...
        self.time.append( dt.currentMSecsSinceEpoch() ) #каждый цикл добавляем в этот массив значение времени в миллисекундах
        ...
        for curve in curves:
            ...
            for i in range(len(self.data[n])):
                new_data.append((curr_curve.max - curr_curve.min)*self.data[n][i] + curr_curve.min) #новый массив это старый (нормализованный) но у которго максимум и минимум не 1 и о а максимум и минимум curr_curve
        ...
        headers = ['time','beam current','vacuum high','T yarmo','T plastik','current 2','gas flow','vacuum tube','vacuum low']
        if len(self.hist)==0:
